<template>
    <div id="multiview" v-bind:style="style">
			<div id="iframeDiv" v-bind:width="width" v-bind:height="height" v-bind:style="style">
          ...
          <div id="video" v-bind:style="iframeStyle[index]">
            <div class="d-flex flex-column" style="border:1px gray solid; border-radius:5px;">
              ...
              <div class="d-flex justify-content-end">
                ...
                <div class="d-flex">
                  <b-button variant="outline-secondary" size="sm" v-bind:id="index" @click="deleteIframe">
...
export default {
  ...
  data () {
    return {
      ...
      style: 'width:' + window.innerWidth + 'px; height:' + (window.innerHeight-60) + 'px;',
  ...
  methods : {
    resizeEvent(event) {
      ...
      this.style = 'width:' + this.width + 'px; height:' + (this.height-60) + 'px;';
      
      this.changeSize();
    },
    addIframe(video, idx) {
      this.iframes.push(video);
      this.youtubePath.push("https://www.youtube.com/embed/"+video.videoId);
      this.changeSize();
    },
    changeSize() {
      if (this.width >= this.height) {
        ...
        for (var i = 0; i < this.iframeLayout.length; i++) {
          for (var j = 0; j < this.iframeLayout[i].length; j++) {
            ...
            this.iframeLayout[i][j][1] = this.stdH * sizeArr[i][j][1];
            this.iframePos[i][j][0] = this.stdW * posArr[i][j][0];
            this.iframePos[i][j][1] = this.stdH * posArr[i][j][1];
          }
        }
        var numOfVideos = this.iframes.length-1; 
        var lay = this.iframeLayout[numOfVideos];
    ...
    deleteIframe(event) {
      ...
      this.youtubePath.splice(index, 1);
      this.changeSize();
    }
  }
}
